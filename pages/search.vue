...
<template v-if="!loading">
  ...
  <div v-else class="space-y-6">
    ...
    <!-- Pagination -->
    <div class="flex flex-wrap items-center justify-center gap-2 pt-6 text-sm font-medium text-maroon">
      <button @click="goToPage(currentPage - 1)" :disabled="currentPage === 1" class="px-2 py-1 hover:underline disabled:opacity-40">
        Prev
      </button>
      ...
      <button
        ...
        :key="n"
        @click="goToPage(n)"
        :class="['h-8 w-8 border transition', n === currentPage ? 'bg-gray-200 text-maroon' : 'hover:underline']"
      >
        {{ n }}
      </button>

      <button @click="goToPage(currentPage + 1)" :disabled="currentPage === totalPages" class="px-2 py-1 hover:underline disabled:opacity-40">
        Next
      </button>
      <button @click="goToPage(totalPages)" :disabled="currentPage === totalPages" class="px-2 py-1 hover:underline disabled:opacity-40">
        End
      </button>
    </div>
  </div>
...
const totalPages = computed(() =>
  Math.max(1, Math.ceil(allResults.value.length / itemsPerPage))
);
...
watchEffect(async () => {
  ...
  try {
      ...
      for (const p of pools) {
        ...
        if (hay.includes(needle)) {
          ...
          bucket.push({
            ...
            snippet: truncate(
              [p.designation, stripHtml(p.educationHtml)]
                .filter(Boolean)
                .join(" — ")
            ),
          });
    ...
    for (const d of depSnap.docs) {
      ...
      if (head && JSON.stringify(head).toLowerCase().includes(needle)) {
        ...
        bucket.push({
          ...
          snippet: truncate(
            [head.designation, stripHtml(head.educationHtml)]
              .filter(Boolean)
              .join(" — ")
          ),
        });
        ...
        for (const s of data.staff) {
          if (JSON.stringify(s).toLowerCase().includes(needle)) {
            ...
            bucket.push({
              ...
              tag: "Department Personnel",
              snippet: truncate(
                [s.designation, stripHtml(s.educationHtml)]
                  .filter(Boolean)
                  .join(" — ")
              ),
            });
    ...
    for (const p of progGroupSnap.docs) {
      ...
      const hay = `${stripHtml(pdata?.content)} ${pdata?.title || ""} ${pdata?.name || ""}`.toLowerCase();
      if (hay.includes(needle)) {
        bucket.push({
