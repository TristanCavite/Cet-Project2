<template>
  <main class="overflow-hidden">
      ...
      <div class="relative w-full h-auto overflow-hidden">
        <!-- Arrows -->
        <button
          class="absolute z-10 flex items-center justify-center h-16 transition transform -translate-y-1/2 border shadow-lg right-4 top-1/2 md:h-28 rounded-xl border-white/20 bg-white/10 backdrop-blur-md hover:bg-white/20"
          @click="nextSlide"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="text-black lucide lucide-chevron-right size-5 md:size-10"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            stroke-width="2"
        ...
        <button
          class="absolute z-10 flex items-center justify-center h-16 transition transform -translate-y-1/2 border shadow-lg left-4 top-1/2 md:h-28 rounded-xl border-white/20 bg-white/10 backdrop-blur-md hover:bg-white/20"
          @click="prevSlide"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="text-black lucide lucide-chevron-left size-5 md:size-10"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            stroke-width="2"
        ...
        <div class="absolute z-10 flex space-x-2 transform -translate-x-1/2 bottom-2 md:bottom-4 left-1/2">
          <span
            ...
            :key="index"
            class="bg-gray-400 rounded-full size-2 md:size-3"
        ...
        >
          <div v-for="(image, index) in images" :key="index" class="flex-shrink-0 w-full h-auto">
            <img
              :src="image.src"
              alt=""
      ...
      <div class="w-full bg-stone-200">
        ...
        <div class="pt-4 text-center">
          <h2 class="text-xl font-extrabold tracking-wide uppercase md:text-5xl text-maroon">EVENTS</h2>
        </div>

        <div class="flex flex-col justify-center gap-8 md:px-6 md:flex-row lg:px-12">
          <!-- 📅 Events or Empty State -->
          <div class="flex flex-col items-center w-full gap-10 pt-5 md:max-w-screen-lg">
            <template v-if="filteredEvents.length > 0">
              <div
                v-for="event in filteredEvents"
                :key="event.id"
                class="w-full p-5 border-2 border-t-2 shadow-xl md:p-10 md:w-224 bg-neutral-50"
              >
                <!-- Date -->
                <span class="font-semibold text-red-800 text-md md:text-2xl font-roboto">
                  DAY
                  {{
                    new Date(event.date)
                ...
                <div class="relative overflow-hidden">
                  ...
                  >
                    <div v-for="(img, i) in event.coverImages" :key="i" class="flex-shrink-0 w-full">
                      <img :src="img" alt="" class="object-cover w-full h-48 md:h-128 md:w-208" />
                    </div>
                  </div>

                  <!-- Arrows -->
                  <button
                    class="absolute z-10 text-gray-800 transition transform -translate-y-1/2 rounded-full shadow-md md:size-10 size-8 right-3 top-1/2 bg-white/80 hover:scale-105 hover:bg-white"
                  ...
                  <button
                    class="absolute z-10 text-gray-800 transition transform -translate-y-1/2 rounded-full shadow-md md:size-10 size-8 left-3 top-1/2 bg-white/80 hover:scale-105 hover:bg-white"
                ...
                <!-- Title & Description -->
                <div class="pb-2 md:pb-4 md:pt-2">
                  <span class="text-xl font-semibold md:text-2xl font-montserrat">{{ event.title }}</span>
                </div>
                ...
                <router-link
                  :to="`/events/${event.id}`"
                  class="inline-block px-3 py-1 mt-2 text-sm font-semibold text-white transition bg-red-800 rounded shadow md:mt-4 md:py-2 md:px-5 hover:bg-red-900"
                >
                  Read more
                </router-link>
              </div>
          ...
          <!--  Right Side -->
          <div class="flex flex-col gap-10 pt-10 pb-5 md:pb-0">
            <!-- Calendar -->
            <div class="pt-5 pb-5 shadow-xl bg-neutral-300">
            ...
            <div class="p-6 space-y-4 bg-white border shadow-md w-96 rounded-xl border-neutral-200 font-roboto">
              ...
              <a
                ...
                class="inline-flex items-center justify-center px-3 py-1 mt-2 text-sm font-medium text-white transition rounded-md shadow md:px-4 md:py-2 bg-maroon hover:bg-red-900"
              >
                Visit Page
              </a>
            </div>
